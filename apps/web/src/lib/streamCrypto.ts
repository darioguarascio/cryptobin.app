...
export async function decryptStreamFrame(key: string, frame: StreamFrame): Promise<string> {
  const cryptoKey = await importStreamKey(key);
  const decrypted = await crypto.subtle.decrypt(
    { name: AES_GCM, iv: toArrayBuffer(base64UrlToBytes(frame.iv)) },
    cryptoKey,
    toArrayBuffer(base64UrlToBytes(frame.ciphertext)),
  );
